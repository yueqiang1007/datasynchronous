...
public class BigQueueUnitTest {
	...
	public void simpleTest() throws IOException {
		...
		for(int i = 1; i <= 2; i++) {
			...
			for(int j = 1; j <= 3; j++) {
				...
				bigQueue.enqueue("hello".getBytes());//将该数据添加到队列后面
				assertTrue(bigQueue.size() == 1L);
				assertTrue(!bigQueue.isEmpty());//确定队列是否为空
				assertEquals("hello", new String(bigQueue.peek()));
				assertEquals("hello", new String(bigQueue.dequeue()));
				assertNull(bigQueue.dequeue());

                bigQueue.enqueue("world".getBytes());//将该数据添加到队列后面

				bigQueue.flush();//强制保持队列的当前状态，

				assertTrue(bigQueue.size() == 1L);
				assertTrue(!bigQueue.isEmpty());

				assertEquals("world", new String(bigQueue.dequeue()));//删除数据  检索和删除队列的前端
				assertNull(bigQueue.dequeue());
			}
			bigQueue.close();//该流并释放任何与系统相关的资源
		}
	}
    ...
    public void simpleTest1() throws IOException {
      ...
      /*  assertNotNull(bigQueue);//线程阻塞
        ...
        assertNull(bigQueue.peek());*/
                bigQueue.enqueue("hello".getBytes());//将该数据添加到队列后面
                bigQueue.enqueue("world".getBytes());//将该数据添加到队列后面
                bigQueue.enqueue("world1".getBytes());//将该数据添加到队列后面
                bigQueue.enqueue("world2".getBytes());//将该数据添加到队列后面
                bigQueue.enqueue("world3".getBytes());//将该数据添加到队列后面
                bigQueue.enqueue("world4".getBytes());//将该数据添加到队列后面
    ...
    public void simpleTest2() throws IOException{
        ...
        System.out.println(bigQueue.size());
        bigQueue.removeAll();
    }
	...
	public void bigLoopTest() throws IOException {
		bigQueue = new BigQueueImpl(testDir, "big_loop_test");//：一个大的、快速的、持久的队列实现，
		...
		for(int i = 0; i < loop; i++) {
			...
			assertTrue(!bigQueue.isEmpty());
			byte[] data = bigQueue.peek();//在队列的前面检索项目
    ...
    public void testApplyForEachDoNotChangeTheQueue() throws Exception {
        ...
        bigQueue.enqueue("卡咖啡可".getBytes());
      ...
      byte[] by =  bigQueue.dequeue();
       ...
       /* assertEquals(4, bigQueue.size());
        assertEquals(bigQueue.size(), dii.getCount());//不相同就阻塞
        ...
        assertArrayEquals("卡咖啡可".getBytes(), bigQueue.dequeue());
        assertEquals(0, bigQueue.size());*/
    }

    @Test
    public void testApplyForEachDoNotChangeTheQueue1() throws Exception {
        bigQueue = new BigQueueImpl(testDir, "testApplyForEachDoNotChangeTheQueue", BigArrayImpl.MINIMUM_DATA_PAGE_SIZE);
       // assertNotNull(bigQueue);
       for (int i=0 ; i < 10 ;i++){
    ...
    public void test1() throws Exception{
        bigQueue = new BigQueueImpl(testDir,"mytest");
        bigQueue.enqueue( "额河口".getBytes() );
    }
